...
import cv2, os, atexit, sys, time, threading
from common_utils import open_camera as _cu_open_camera

# [ADD] FIRE 좌표 전송(고정값) 설정
import socket
    ...
    with _cap_lock:
        if CAM_PATH.strip() == "":
            ...
            try:
                ...
                pass
            src_desc = str(src)
    print(f"[CAM] open src={src_desc} opened={cap.isOpened()} "
          f"W/H/FPS={cap.get(3):.0f}/{cap.get(4):.0f}/{cap.get(5):.0f}")
...
def _looks_like_fire(bgr_frame):
    try:
        hsv = cv2.cvtColor(bgr_frame, cv2.COLOR_BGR2HSV)
        # 빨강/주황 대략 범위
        mask1 = cv2.inRange(hsv, (0, 80, 80), (15, 255, 255))     # 빨강~주황 하한
        mask2 = cv2.inRange(hsv, (160, 80, 80), (180, 255, 255))  # 빨강 상한
        ratio = (mask1.mean() + mask2.mean()) / (2 * 255.0)       # 0.0~1.0
        return ratio > 0.08
    except Exception:
        return False
...
def process_frame(frame):
    # 처리 파이프 표시(좌상단 SRV)
    cv2.putText(frame, "SRV", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 255), 2, cv2.LINE_AA)

    fire_boxes = []
    if callable(detect_and_draw):
        try:
            out = detect_and_draw(frame)  # 반드시 "그려진 BGR 프레임" 반환
            if out is not None:
                frame = out
            try:
    ...
    if fire_boxes:
        try:
            # 감지가 이어지는 동안에도 최소 한 번은 고정 좌표를 보낸다.
            notify_fire_fixed()
        except Exception:
            pass
    else:
    ...
    # 브리지 신호가 전혀 없을 때의 보조 휴리스틱 (서버 전송에는 사용하지 않음)
    if not fire_boxes and _looks_like_fire(frame):
        try:
            # 최소한 UI 표기를 위해 빨간 텍스트만 추가
            cv2.putText(frame, "FIRE?", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2, cv2.LINE_AA)
        except Exception:
            pass
